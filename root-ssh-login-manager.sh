...
write_root_auth_config() {
  ...
  case "$mode" in
    key)
      content=$'# Managed by root-ssh-login-manager.sh\n# Root may sign in only with a public key; other users are unchanged.\nPermitRootLogin prohibit-password\nPubkeyAuthentication yes\n'
      ;;
    password)
      content=$'# Managed by root-ssh-login-manager.sh\n# Enables root SSH password login. PasswordAuthentication is global.\nPermitRootLogin yes\nPasswordAuthentication yes\nPubkeyAuthentication yes\n'
      ;;
    *) die '内部错误：未知认证模式' ;;
  esac
...
ssh_self_check() {
  local effective key_files mode
  clear
  echo '================================================'
  echo '              Root SSH 认证自检'
  echo '================================================'
  find_sshd
  if "$SSHD_BIN" -t -f "$SSHD_MAIN"; then
    ok 'sshd -t：SSH 配置语法有效'
  else
    warn 'sshd -t：SSH 配置存在错误；请勿切换登录方式。'
    pause
    return
  fi

  effective="$("$SSHD_BIN" -T -f "$SSHD_MAIN" -C user=root,host=localhost,addr=127.0.0.1 2>/dev/null || true)"
  [[ -n "$effective" ]] || { warn '无法读取 root 的 SSH 生效配置。'; pause; return; }
  mode="$(printf '%s\n' "$effective" | awk '/^(permitrootlogin|passwordauthentication|pubkeyauthentication) / {print}')"
  echo "$mode"
  key_files="$(printf '%s\n' "$effective" | awk '/^authorizedkeysfile / { $1=""; sub(/^ /, ""); print }')"
  echo "authorizedkeysfile $key_files"
  echo "有效 root 公钥数量：$(root_key_count)"

  if root_authorized_keys_file_enabled; then
    ok "sshd 会读取 $AUTHORIZED_KEYS"
  else
    warn "当前 sshd 的 AuthorizedKeysFile 不包含 .ssh/authorized_keys；导入到 $AUTHORIZED_KEYS 的密钥不会用于 root 登录。"
  fi
  if [[ -f "$AUTHORIZED_KEYS" ]]; then
    printf '权限：.ssh=%s，authorized_keys=%s\n' "$(stat -c '%a' "$SSH_DIR" 2>/dev/null || echo '?')" "$(stat -c '%a' "$AUTHORIZED_KEYS" 2>/dev/null || echo '?')"
  fi
  pause
}
import_github_keys() {
  ...
  if (( $(root_key_count) > 0 )) && root_authorized_keys_file_enabled; then
    echo
    warn '下一步会禁止 root 使用 SSH 密码登录。务必先确认该 GitHub 公钥对应的私钥可用。'
    if confirm '确认切换 root 为仅密钥登录？'; then
      write_root_auth_config key
    else
      info '已保留当前 root SSH 登录方式。'
    fi
  fi
}
...
key_login_menu() {
  while true; do
    ...
    echo '------------------------------------------------'
    echo '1. 从 GitHub 导入已有公钥'
    echo '2. 启用 root 仅密钥登录（禁止 root SSH 密码登录）'
    echo '3. 查看 root 已授权公钥'
    echo '0. 返回主菜单'
    echo '------------------------------------------------'
    ...
    case "$choice" in
      ...
      2)
        (( $(root_key_count) > 0 )) || { warn '没有 root 公钥，不能安全地禁止 root 密码登录。'; pause; continue; }
        root_authorized_keys_file_enabled || { warn 'sshd 未配置读取 /root/.ssh/authorized_keys，拒绝禁用 root 密码登录。'; pause; continue; }
        warn '请先在新的 SSH 终端确认密钥登录成功，再继续。'
        if confirm '确认禁止 root 的 SSH 密码登录？'; then write_root_auth_config key; fi
        pause ;;
      3) [[ -f "$AUTHORIZED_KEYS" ]] && cat "$AUTHORIZED_KEYS" || info '暂无已授权公钥'; pause ;;
      0) return ;;
      *) warn '无效选择'; pause ;;
    esac
  done
...
password_login_menu() {
  ...
  echo '             Root 密码登录管理'
  echo '================================================'
  warn '此操作会允许 root 使用 SSH 密码登录，并会全局启用 PasswordAuthentication。'
  warn '密码登录会增加暴力破解风险，建议设置强密码并限制防火墙来源 IP。'
  if ! confirm '继续设置 root 密码并开启密码登录？'; then
    return
  fi
  if ! passwd root; then
