[Uploading root-ssh-login-manager.sh…]()
#!/usr/bin/env bash
...
ssh_self_check() {
  ...
  if [[ "$permit" == 'prohibit-password' || "$permit" == 'without-password' ]]; then
    current_mode='root 密钥登录（已禁止 root SSH 密码登录）'
  elif [[ "$permit" == 'yes' && "$password" == 'yes' ]]; then
    current_mode='root SSH 密码登录（同时允许密钥登录）'
  elif [[ "$permit" == 'no' ]]; then
  ...
  echo "当前登录模式：$current_mode"
  echo "root 密钥登录：$([[ "$pubkey" == 'yes' ]] && echo '已启用' || echo '未启用')"
  echo "root 密码登录：$([[ "$permit" == 'yes' && "$password" == 'yes' ]] && echo '已启用' || echo '已禁止')"
